from .applications_request_new import ApplicationsRequestNew
from .client_customization_request_new import ClientCustomizationRequestNew
from .data_leak_prevention_request_new import DataLeakPreventionRequestNew
from .harmony_mobile_request_new import HarmonyMobileRequestNew
from .pydantic import BaseModel, Field
from .security_request_new import SecurityRequestNew


class MobileProfileRequestNew(BaseModel):
    applications: ApplicationsRequestNew = Field(
        alias="applications", description="""Applications settings."""
    )
    client_customization: ClientCustomizationRequestNew = Field(
        alias="client-customization", description="""Client customization settings."""
    )
    data_leak_prevention: DataLeakPreventionRequestNew = Field(
        alias="data-leak-prevention", description="""Data leak prevention settings."""
    )
    harmony_mobile: HarmonyMobileRequestNew = Field(
        alias="harmony-mobile", description="""Integrations settings."""
    )
    security: SecurityRequestNew = Field(alias="security", description="""Security settings.""")
