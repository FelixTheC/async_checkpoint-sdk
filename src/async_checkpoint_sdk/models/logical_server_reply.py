...
class LogicalServerReply(BaseModel):
    ...
    server_group: ApiObjectStandardIdentifier = Field(
        alias="server-group",
        description="""Server group associated with the logical server. Level of details in the output corresponds to the number of details for search. This table shows the level of details in the Standard level.""",
    )
    ...
    persistency_type: str = Field(
        alias="persistency-type", description="""Persistency type for the logical server."""
    )
    balance_method: str = Field(
        alias="balance-method", description="""Load balancing method for the logical server."""
    )
    color: str = Field(
