...
class CentralLicensesReply(BaseModel):
    central: bool = Field(
        alias="central", description="""Indicates whether the license is central or local."""
    )
    ck: str = Field(
        alias="ck", description="""A unique identifier which represent a specific license."""
    )
    ...
    additional_info: LicenseInfoReply = Field(
        alias="additional-info", description="""Additional info for cloud licenses (ve licenses)."""
    )
