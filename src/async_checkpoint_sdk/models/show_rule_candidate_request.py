...
class ShowRuleCandidateRequest(BaseModel):
    category_tags: list[str] = Field(alias="category-tags", description="""N/A""")
    field: str = Field(
        alias="field",
    ...
    filter: str = Field(
        alias="filter", description="""Textual search expression to filter objects by."""
    )
    layer: str = Field(alias="layer", description="""Layer or exception group name or uid.""")
