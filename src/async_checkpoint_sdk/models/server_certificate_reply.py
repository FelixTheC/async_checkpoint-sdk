...
class ServerCertificateReply(BaseModel):
    ...
    issued_by: str = Field(
        alias="issued-by", description="""The DN (Distinguished Name) of the certificate."""
    )
    subject: str = Field(alias="subject", description="""Certificate's subject.""")
    valid_from: str = Field(
        alias="valid-from", description="""Server certificate valid from date."""
    )
    valid_to: str = Field(alias="valid-to", description="""Server certificate valid up to date.""")
    color: str = Field(
