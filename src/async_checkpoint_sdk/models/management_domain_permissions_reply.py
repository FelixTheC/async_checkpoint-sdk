...
class ManagementDomainPermissionsReply(BaseModel):
    ...
    approve_or_reject_sessions: bool = Field(
        alias="approve-or-reject-sessions", description="""Approve / reject other sessions."""
    )
    publish_sessions: bool = Field(
        alias="publish-sessions", description="""Allow session publishing without an approval."""
    )
