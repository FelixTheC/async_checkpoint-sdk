...
class IndicatorShowReply(BaseModel):
    name: str = Field(alias="name", description="""Object name. Must be unique in the domain.""")
    uid: str = Field(alias="uid", description="""Object unique identifier.""")
    type: str = Field(alias="type", description="""Object type.""")
    action: str = Field(alias="action", description="""The indicator's action.""")
    number_of_observables: int = Field(
        alias="number-of-observables", description="""The amount of observables in the indicator."""
    )
    observables_raw_data: str = Field(
        alias="observables-raw-data",
        description="""The contents of the file containing the indicator's observables.<br>Available only when details-level is full.""",
    )
    profile_overrides: list[dict] = Field(
