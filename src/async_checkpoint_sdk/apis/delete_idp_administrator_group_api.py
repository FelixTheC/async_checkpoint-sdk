...
async def delete_idp_administrator_group(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing Identity Provider administrators group using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-idp-administrator-group"
    data_obj = {"body": data}
