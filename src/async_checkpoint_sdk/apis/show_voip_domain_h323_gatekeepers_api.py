...
async def show_voip_domain_h323_gatekeepers(
    client: ClientSession, data: ApiQueryRequest, config: Config, **kwargs
) -> ApiQueryObjectReply:
    """
    Retrieve all voip VoIP Domain H.323 Gatekeepers.
    ...
    ApiQueryObjectReply
    """
    url = f"https://{config.server}:{config.port}/web_api/show-voip-domain-h323-gatekeepers"
    data_obj = {"body": data}
