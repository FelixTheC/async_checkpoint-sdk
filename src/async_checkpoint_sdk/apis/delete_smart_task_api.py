...
async def delete_smart_task(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing object using object name or uid. <br>This command is available only in a Security Management environment or in Multi-Domain environment when logged into local domain.
    ...
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-smart-task"
    data_obj = {"body": data}
