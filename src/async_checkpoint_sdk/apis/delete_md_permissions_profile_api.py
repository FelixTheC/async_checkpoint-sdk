...
async def delete_md_permissions_profile(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing Multi-Domain Permissions Profile using object name or uid.
    ...
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-md-permissions-profile"
    data_obj = {"body": data}
