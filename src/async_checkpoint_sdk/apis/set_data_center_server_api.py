...
async def set_data_center_server(
    client: ClientSession, data: DataCenterServerRequestEdit, config: SDKConfig, **kwargs
) -> DataCenterServerTaskReply:
    """
    Edit existing Data Center Server using name or uid.<br> Data Center Server represents the connection to a cloud environment.<br> The Data Center Server contains Data Center Objects, these objects can be imported from it using the add-data-center-object command.<span class="show-only-in-doc-ui"><br><span class="show-only-in-doc-ui"><span style="color: red;">Note:</span> Each Data Center Server type uses additional dedicated arguments, <a data-toggle="modal" href="#" data-target="#DataC
    Parameters
    ----------
    ...
    -------
    DataCenterServerTaskReply.
    data : DataCenterServerRequestEdit
        data : DataCenterServerRequestEdit [Argument].
    config : SDKConfig
    kwargs : Any
        Keyword arguments
