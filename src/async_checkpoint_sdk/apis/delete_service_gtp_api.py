...
async def delete_service_gtp(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing GTP service object using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-service-gtp"
    data_obj = {"body": data}
