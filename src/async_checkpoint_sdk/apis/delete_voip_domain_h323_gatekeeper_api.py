...
async def delete_voip_domain_h323_gatekeeper(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing VoIP Domain H.323 Gatekeeper using object name or uid.
    ...
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-voip-domain-h323-gatekeeper"
    data_obj = {"body": data}
