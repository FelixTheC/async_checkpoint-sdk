...
async def delete_voip_domain_mgcp_call_agent(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing VoIP Domain MGCP Call Agent using object name or uid.
    ...
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-voip-domain-mgcp-call-agent"
    data_obj = {"body": data}
