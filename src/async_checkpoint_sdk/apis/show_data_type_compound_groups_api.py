...
async def show_data_type_compound_groups(
    client: ClientSession, data: ApiQueryRequest, config: Config, **kwargs
) -> ApiQueryObjectReply:
    """
    Retrieve all Compound Data Type Group objects.
    ...
    ApiQueryObjectReply
    """
    url = f"https://{config.server}:{config.port}/web_api/show-data-type-compound-groups"
    data_obj = {"body": data}
