...
async def delete_voip_domain_h323_gateway(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing VoIP Domain H.323 Gateway using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-voip-domain-h323-gateway"
    data_obj = {"body": data}
