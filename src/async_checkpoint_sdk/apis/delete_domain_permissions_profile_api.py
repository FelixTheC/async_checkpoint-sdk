...
async def delete_domain_permissions_profile(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing Domain Permissions Profile using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-domain-permissions-profile"
    data_obj = {"body": data}
