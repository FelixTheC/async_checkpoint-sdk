...
async def delete_resource_uri_for_qos(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing Uri For QoS resource using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-resource-uri-for-qos"
    data_obj = {"body": data}
