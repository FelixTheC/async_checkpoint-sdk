...
async def delete_voip_domain_sip_proxy(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: SDKConfig, **kwargs
) -> ApiOkReply:
    """
    Delete existing VoIP Domain SIP Proxy using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-voip-domain-sip-proxy"
    data_obj = {"body": data}
