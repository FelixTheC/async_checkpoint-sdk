...
async def show_infinity_idps(
    client: ClientSession, data: ApiQueryRequest, config: SDKConfig, **kwargs
) -> ApiQueryObjectReply:
    """
    Retrieve all Infinity Identity Provider from the Infinity Portal.
    ...
    """
    url = f"https://{config.server}:{config.port}/web_api/show-infinity-idps"
    data_obj = {"body": data}
