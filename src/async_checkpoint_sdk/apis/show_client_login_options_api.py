...
async def show_client_login_options(
    client: ClientSession, data: ApiQueryRequest, config: SDKConfig, **kwargs
) -> ApiQueryObjectReply:
    """
    Retrieve multiple client login options using various filters.
    ...
    ApiQueryObjectReply
    """
    url = f"https://{config.server}:{config.port}/web_api/show-client-login-options"
    data_obj = {"body": data}
