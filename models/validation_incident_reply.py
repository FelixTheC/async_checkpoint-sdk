...
class ValidationIncidentReply(BaseModel):
    creation_time: ApiDateReply = Field(
        alias="creation-time", description="""Creation time."""
    )
    current_session: bool = Field(
        alias="current-session",
    ...
    related_objects: list[dict] = Field(
        alias="related-objects", description="""Related objects."""
    )
