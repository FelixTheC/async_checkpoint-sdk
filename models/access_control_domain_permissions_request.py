from access_control_policy_layers_domain_permissions_request import (
    AccessControlPolicyLayersDomainPermissionsRequest,
)
from pydantic import BaseModel, Field


class AccessControlDomainPermissionsRequest(BaseModel):
    ...
    policy_layers: AccessControlPolicyLayersDomainPermissionsRequest = Field(
        alias="policy-layers",
        description="""Layer editing permissions.<br>Available only if show-policy is set to true.""",
    )
    dlp_policy: str = Field(
        alias="dlp-policy", description="""Configure DLP rules and Policies."""
    )
    ...
    qos_policy: str = Field(
        alias="qos-policy", description="""Work with QoS Policies and rules."""
    )
