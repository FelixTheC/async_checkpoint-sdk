...
class UriResourceRequestEdit(BaseModel):
    ...
    match_ufp: UfpObjectRequest = Field(
        alias="match-ufp", description="""Match - UFP settings."""
    )
    match_wildcards: UriResourceWildcardsRequest = Field(
        alias="match-wildcards", description="""Match - Wildcards settings."""
    )
    action: UriResourceActionRequest = Field(
        alias="action", description="""Action settings."""
    )
