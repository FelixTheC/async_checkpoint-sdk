from default_mep_priority_rule_request_new import DefaultMepPriorityRuleRequestNew
from mep_priority_rule_request_new import MepPriorityRuleRequestNew
from pydantic import BaseModel, Field


class MultipleEntryPointsRequestNew(BaseModel):
    enabled: bool = Field(
        alias="enabled",
    ...
    tracking: str = Field(
        alias="tracking", description="""Tracking option for the MEP."""
    )
    default_priority_rule: DefaultMepPriorityRuleRequestNew = Field(
    ...
    exception_priority_rules: MepPriorityRuleRequestNew | list[dict] = Field(
