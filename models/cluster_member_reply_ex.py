...
class ClusterMemberReplyEx(BaseModel):
    ...
    domain: ApiDomainIdentifier = Field(
        alias="domain", description="""Domain information."""
    )
    interfaces: list[dict] = Field(
        alias="interfaces", description="""Cluster member network interfaces."""
    )
    ip_address: str = Field(
        alias="ip-address", description="""Cluster member IP address."""
    )
    ipv6_address: str = Field(
        alias="ipv6-address", description="""Cluster member IPv6 address."""
    )
