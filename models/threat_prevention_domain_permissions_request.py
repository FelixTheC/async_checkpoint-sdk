...
class ThreatPreventionDomainPermissionsRequest(BaseModel):
    ...
    profiles: str = Field(
        alias="profiles", description="""Configure Threat Prevention profiles."""
    )
    protections: str = Field(
        alias="protections", description="""Work with malware protections."""
    )
    install_policy: bool = Field(
        alias="install-policy", description="""Install Policies."""
    )
