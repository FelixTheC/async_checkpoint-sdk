...
class UserRequestNew(BaseModel):
    template: str = Field(
        alias="template", description="""User template name or UID."""
    )
    ...
    from_hour: str = Field(
        alias="from-hour", description="""Allow users connect from hour."""
    )
    to_hour: str = Field(
        alias="to-hour", description="""Allow users connect until hour."""
    )
