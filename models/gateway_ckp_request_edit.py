...
from firewall_settings_request_edit import FirewallSettingsRequestEdit
from identity_awareness_settings_request_edit import (
    IdentityAwarenessSettingsRequestEdit,
)
from ips_settings_request_edit import IpsSettingsRequestEdit
...
class GatewayCkpRequestEdit(BaseModel):
    ...
    autonomous_system_number: str = Field(
        ...
        description="""The Autonomous System Number (ASN). It is automatically fetched from the Security Gateway object. You can change this value only for externally managed devices.""",
    )
    ...
    firewall_settings: FirewallSettingsRequestEdit = Field(
    ...
    interfaces: list[dict] = Field(
        alias="interfaces",
        description="""Network interfaces. When a gateway is updated with a new interfaces, the existing interfaces are removed.""",
    )
...
Changing this setting is supported only for Quantum Spark gateways.""",
    ...
    ips_settings: IpsSettingsRequestEdit = Field(
    ...
    mobile_access: bool = Field(
        alias="mobile-access", description="""Mobile Access blade."""
    )
    ...
    platform_portal_settings: PlatformPortalRequestEdit = Field(
    ...
    usercheck_portal_settings: UserCheckPortalRequestEdit = Field(
    ...
    vpn_settings: VpnSettingsRequestEdit = Field(
