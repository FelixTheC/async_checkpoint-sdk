...
class SecurityGroupMemberInfoReply(BaseModel):
    appliance_model: str = Field(
        alias="appliance-model",
        description="""Appliance model of the Security Group Member.""",
    )
    cpu_usage: int = Field(
        alias="cpu-usage", description="""CPU Usage (%) of the Security Group Member."""
    )
    disk_space_status: str = Field(
        alias="disk-space-status",
        description="""Disk space status of current member.""",
    )
    ...
    member_id: int = Field(
        alias="member-id",
        description="""ID of the Security Group Member on the Maestro Site.""",
    )
    member_ip: str = Field(
        alias="member-ip", description="""Sync IP of current member."""
    )
    ...
    memory_usage: int = Field(
        alias="memory-usage",
        description="""Memory usage (%) of the Security Group Member.""",
    )
    ...
    throughput: int = Field(
        alias="throughput",
        description="""Total throughput of the Security Group Member.""",
    )
    version: str = Field(
        alias="version",
        description="""Software version installed on the Security Group Member.""",
    )
