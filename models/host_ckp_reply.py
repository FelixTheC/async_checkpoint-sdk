...
class HostCkpReply(BaseModel):
    ...
    nat_settings: ThirdPartyNatReply = Field(
    ...
    sic_name: str = Field(
        alias="sic-name", description="""Secure Internal Connection name."""
    )
    sic_state: str = Field(
        alias="sic-state", description="""Secure Internal Connection trust state."""
    )
    version: str = Field(
        alias="version", description="""Check Point host platform version."""
    )
