...
from host_servers_reply import HostServersReply
...
class HostReply(BaseModel):
    ...
    ipv4_address: str = Field(
        alias="ipv4-address", description="""IPv4 host address."""
    )
    ipv6_address: str = Field(
        alias="ipv6-address", description="""IPv6 host address."""
    )
    nat_settings: NatSettingsReply = Field(
        alias="nat-settings", description="""NAT settings."""
    )
    host_servers: HostServersReply = Field(
