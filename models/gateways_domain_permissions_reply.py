...
class GatewaysDomainPermissionsReply(BaseModel):
    ...
    lsm_gw_db: str = Field(
        alias="lsm-gw-db",
        description="""Access to objects defined in LSM gateway tables. These objects are managed in the SmartProvisioning GUI or LSMcli command-line.<br>Note: 'Write' permission on lsm-gw-db allows administrator to run a script on LSM gateway in Expert mode.""",
    )
    ...
    system_backup: bool = Field(
        alias="system-backup", description="""Backup Security Gateways."""
    )
