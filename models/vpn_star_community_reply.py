...
from multiple_entry_points_reply import MultipleEntryPointsReply
from pydantic import BaseModel, Field
from route_based_settings_reply import RouteBasedSettingsReply
from star_encrypted_traffic_reply import StarEncryptedTrafficReply
from star_permanent_tunnels_reply import StarPermanentTunnelsReply
from wire_mode_reply import WireModeReply


class VpnStarCommunityReply(BaseModel):
    name: str = Field(
        alias="name", description="""Object name. Must be unique in the domain."""
    )
    uid: str = Field(alias="uid", description="""Object unique identifier.""")
    type: str = Field(alias="type", description="""Object type.""")
    center_gateways: list[dict] = Field(
        alias="center-gateways",
        description="""Collection of center VPN Gateway and VPN Device objects identified by the name or UID. Level of details in the output corresponds to the number of details for search. This table shows the level of details in the Standard level.""",
    )
    ...
    encrypted_traffic: StarEncryptedTrafficReply = Field(
    ...
    mep: MultipleEntryPointsReply = Field(
    ...
    permanent_tunnels: StarPermanentTunnelsReply = Field(
        alias="permanent-tunnels", description="""Permanent tunnels properties."""
    )
    satellite_gateways: list[dict] = Field(
