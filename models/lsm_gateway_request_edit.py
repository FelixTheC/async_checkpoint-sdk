...
class LsmGatewayRequestEdit(BaseModel):
...
If 'using-profile' state is provided a provisioning profile must be provided in provisioning-settings.""",
    ...
    security_profile: str = Field(
        alias="security-profile", description="""LSM profile."""
    )
    sic: LsmGatewaySicRequest = Field(
        alias="sic", description="""Secure Internal Communication."""
    )
