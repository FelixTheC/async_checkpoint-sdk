...
async def delete_voip_domain_sccp_call_manager(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing VoIP Domain SCCP Call Manager using object name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-voip-domain-sccp-call-manager"
    data_obj = {"body": data}
