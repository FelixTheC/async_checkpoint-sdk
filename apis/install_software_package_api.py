from aiohttp import ClientSession

from config import Config
from models.deployment_command_reply import DeploymentCommandReply
from models.deployment_command_request_install import DeploymentCommandRequestInstall


async def install_software_package(
    client: ClientSession, data: DeploymentCommandRequestInstall, config: Config, **kwargs
) -> DeploymentCommandReply:
    """
    Installs the software package on target machines.
    
    Parameters
    ----------
    client : ClientSession [Argument]
    data : DeploymentCommandRequestInstall [Argument]
    config : Config [Argument]
    kwargs : [Keyword arguments]

    Returns
    -------
    DeploymentCommandReply
    """
    url = f"https://{config.server}:{config.port}/web_api/install-software-package"
    data_obj = {"body": data}
    if client.headers["Content-Type"] == "application/json":
        data_obj = {"json": data}
    async with client.post(url, **data_obj, raise_for_status=True, ssl=False) as response:
        resp = await response.json()
    return DeploymentCommandReply(**resp)
