from config import Config
from aiohttp import ClientSession
from models.track_settings_request import TrackSettingsRequest
from models.track_settings_reply import TrackSettingsReply


async def show_access_rule_track_settings(
    client: ClientSession, data: TrackSettingsRequest, config: Config, **kwargs
) -> TrackSettingsReply:
    """ 
    Parameters
    ----------
    client : ClientSession [Argument]
    data : TrackSettingsRequest [Argument]
    config : Config [Argument]
    kwargs : [Keyword arguments]

    Returns
    -------
    TrackSettingsReply
    """
    url = f"https://{config.server}:{config.port}/web_api/show-access-rule-track-settings"
    data_obj = {"body": data}
    if client.headers["Content-Type"] == "application/json":
        data_obj = {"json": data}
    async with client.post(url, **data_obj, raise_for_status=True, ssl=False) as response:
        resp = await response.json()
    return TrackSettingsReply(**resp)
