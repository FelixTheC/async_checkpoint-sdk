from config import Config
from aiohttp import ClientSession
from models.sccp_call_manager_reply import SccpCallManagerReply
from models.sccp_call_manager_request_edit import SccpCallManagerRequestEdit


async def clone_voip_domain_sccp_call_manager(
    client: ClientSession, data: SccpCallManagerRequestEdit, config: Config, **kwargs
) -> SccpCallManagerReply:
    """
    Clone existing VoIP Domain SCCP Call Manager.
    
    Parameters
    ----------
    client : ClientSession [Argument]
    data : SccpCallManagerRequestEdit [Argument]
    config : Config [Argument]
    kwargs : [Keyword arguments]

    Returns
    -------
    SccpCallManagerReply
    """
    url = f"https://{config.server}:{config.port}/web_api/clone-voip-domain-sccp-call-manager"
    data_obj = {"body": data}
    if client.headers["Content-Type"] == "application/json":
        data_obj = {"json": data}
    async with client.post(url, **data_obj, raise_for_status=True, ssl=False) as response:
        resp = await response.json()
    return SccpCallManagerReply(**resp)
