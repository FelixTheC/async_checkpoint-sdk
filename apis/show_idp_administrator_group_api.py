...
async def show_idp_administrator_group(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestShow, config: Config, **kwargs
) -> IdpGroupRoleReply:
    """
    Retrieve existing Identity Provider administrators group using object name or uid.
    ...
    IdpGroupRoleReply
    """
    url = f"https://{config.server}:{config.port}/web_api/show-idp-administrator-group"
    data_obj = {"body": data}
