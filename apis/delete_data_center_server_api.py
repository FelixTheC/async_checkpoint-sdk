...
async def delete_data_center_server(
    client: ClientSession, data: ApiVisualCPObjectIdentifierRequestDelete, config: Config, **kwargs
) -> ApiOkReply:
    """
    Delete existing Data Center Server using name or uid.
    ...
    ApiOkReply
    """
    url = f"https://{config.server}:{config.port}/web_api/delete-data-center-server"
    data_obj = {"body": data}
